...
const STORY_PARAGRAPHS = [
  ...
  "I craft digital software with an obsessive focus on tactile materials, spatial flow, and fluid motion physics. Currently exploring spatial computing, interactive Web Audio shaders, and high craft design systems.",
];
...
function ScrollStoryParagraphs({
  progress,
  reduce,
}: {
  progress: MotionValue<number>;
  ...
  const paragraphWords = STORY_PARAGRAPHS.map((p) => p.split(" "));
  const totalWords = paragraphWords.reduce((acc, words) => acc + words.length, 0);

  let globalWordIndex = 0;

  return (
    <div className="flex flex-col gap-3.5 font-display text-[16px] sm:text-[18px] font-medium leading-relaxed sm:leading-[1.65] max-w-[700px] text-pretty">
      {paragraphWords.map((words, pIdx) => (
        <p key={pIdx} className="text-pretty">
          {words.map((word, wIdx) => {
            const currentIdx = globalWordIndex++;
            const start = (currentIdx / totalWords) * 0.92;
...
export function AboutHeroSection() {
  ...
  return (
    <section
      ref={sectionRef}
      className={`relative w-full ${reduce ? "min-h-0 pt-28 sm:pt-36" : "min-h-[160vh] sm:min-h-[185vh]"
        }`}
    >
      <div
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            {/* Scroll-driven Story paragraphs */}
            <motion.div {...fadeUp(0.08)}>
              <ScrollStoryParagraphs progress={maxProgress} reduce={reduce} />
            </motion.div>
